...
class MerlinMainTree(MerlinTree):
    ...
    def populate(self, items, thumbnails, overwrite):
        ...
        for item in items:
            ...
            iid = self.insert(parent, item['order']+offsets[parent],
                              text=item['title'], values=data, image=thumbnails[item['uuid']])
            mapiid[item['id']] = iid
            offsets[iid] = 0
            self.set(iid, 'id', iid)
    ...
    def subtree_to_list(self, node, counter=0, order=0, parent=1):
        ...
        items = [item]
        for order, c in enumerate(children):
            sublist, counter = self.subtree_to_list(
                c, counter, order, item['id'])
            items.extend(sublist)
    ...
    def add_menu(self):
        ...
        iid = self.insert(self.parent(current_node), self.index(
            current_node)+1, text=' \u25AE Nouveau Menu', tags="directory")
        self.set(iid, 'type', '6')
    ...
    def set_node_image(self, node, uuid, filepath):
        with Image.open(filepath) as image:
            image_small = image.resize((40, 40), Image.LANCZOS)
            self.rootGUI.thumbnails[uuid] = ImageTk.PhotoImage(image_small)

        self.item(node, image=self.rootGUI.thumbnails[uuid])
        self.set(node, 'imagepath', filepath)
        self.update()

    def add_sound(self):
        current_node = self.selection()
        playlist_dirname = os.path.dirname(self.rootGUI.playlistpath)
        filepaths = filedialog.askopenfilename(initialdir=playlist_dirname, filetypes=[
                                               ('mp3', '*.mp3')], multiple=True)
        if not filepaths:
            return
        for filepath in filepaths:
            ...
            while len(b) > 64:
                ...
                answer = tk.messagebox.askokcancel(
                    "Nom de fichier trop long", f"Le nom de fichier '{basename}' est trop long.\nLe copier sous un nouveau nom ?")
                if not answer:
                    return
                new_filepath = tk.filedialog.asksaveasfilename(
                    initialdir=dirname, initialfile=new_basename, filetypes=[('mp3', '*.mp3')], multiple=False)
                if not new_filepath:
                    return
            ...
            if new_filepath != filepath:
                ...
                shutil.copyfile(filepath, new_filepath)
            iid = self.insert(self.parent(current_node), self.index(
                current_node)+1, text=' \u266A ' + uuid, tags='sound')
            self.set(iid, 'type', '4')
            ...
            if os.path.exists(image_path):
                print(image_path + " exists, use it!")
                self.set_node_image(iid, uuid, image_path)
        if len(filepaths) == 1:
            self.focus(iid)
            self.selection_set(iid)
        self.update()
    ...
    def select_image(self):
        ...
        filepath = filedialog.askopenfilename(
            initialdir=playlist_dirname, initialfile=initfile, filetypes=[('images jpg', '*.jpg')])
        if not filepath:
            return
        with open(filepath, 'rb') as stream:
            if IsImageProgressive(stream):
                tk.messagebox.showwarning(
                    title="Problème de format", message=f"Le format de l'image est JPEG 'progressive'. Ce format n'est pas pris en charge par toutes les Merlin.")
        dirname, basename = os.path.split(filepath)
        ...
        if self.tag_has('directory', current_node):
            ...
            while len(b) > 64:
                ...
                if not answer:
                    return
                new_filepath = tk.filedialog.asksaveasfilename(
                    initialdir=dirname, initialfile=new_basename, filetypes=[('jpg', '*.jpg')], multiple=False)
                if not new_filepath:
                    return
                new_dirname, new_basename = os.path.split(new_filepath)
        ...
        # else:
            ...
            # filepath = new_filepath
            # dirname, basename = os.path.split(filepath)
        self.set_node_image(current_node, uuid, filepath)
...
class MerlinFavTree(MerlinTree):
    ...
    def populate(self, main_tree, overwrite):
        ...
        fav_list = sorted([(int(main_tree.set(node, 'fav_order')), node) for node in main_tree.tag_has(
            'favorite') if not self.exists(node)], reverse=True)
